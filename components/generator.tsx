...
export function Generator() {
  ...
  async function handleGenerate(data: any) {
    ...
    if (state.plan === 'free' && state.generationsLeft === 0) {
      alert('Trial expired - Subscribe for $4.99');
      return;
    }
  ...
  return (
    <div className="max-w-xl mx-auto p-4 space-y-4">
      <div className="text-xs text-gray-500">
        Trial: {trial.expired ? 'Expired' : `${trial.hours}h ${trial.minutes}m left`}
      </div>
      ...
      {kit && (
        <>
          {/* @ts-ignore - fixes Type error: missing onSave, saved */}
          <OutputTabs kit={kit} onSave={() => {}} saved={false} onCopy={() => {}} />
        </>
      )}
    </div>
  );
