...
function CopyBtn({ text, label = "Copy" }: { text: string; label?: string }) {
  ...
  return (
    <button onClick={handleCopy} className="rounded-full border border-gray-300 bg-white px-4 py-2 text-sm font-bold text-gray-700 hover:bg-gray-50 transition shadow-sm">
      {label}
    </button>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <div className="flex gap-4">
          <div className="flex-shrink-0">
            {kit.productImage ? (
              ...
              <div className="h-32 w-32 rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 flex items-center justify-center">
                <span className="text-xs text-gray-400 text-center px-2">Image Blocked by Amazon</span>
              </div>
            )}
          </div>
          ...
          <div className="flex-1 space-y-3">
            ...
            <a href={kit.affiliateLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center rounded-full bg-yellow-400 px-5 py-2.5 text-sm font-bold text-gray-900 hover:bg-yellow-500 transition shadow-sm">
              🛒 View on Amazon
            </a>
            <div className="flex gap-2">
              <input value={kit.affiliateLink || ''} readOnly className="flex-1 rounded-full bg-gray-100 px-3 py-2 text-xs text-gray-600 truncate border border-gray-200" />
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <div className="flex gap-2">
          <input value={kit.affiliateLink || ''} readOnly className="flex-1 rounded-lg bg-gray-100 px-4 py-2 text-sm text-gray-700 border border-gray-200" />
          <CopyBtn text={kit.affiliateLink || ''} label="Copy" />
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <div className="rounded-xl bg-gray-50 p-4 border border-gray-200">
          <p className="text-sm text-gray-700 leading-relaxed">{kit.hashtags?.join(' ')}</p>
        </div>
      </div>
    </div>
  );
